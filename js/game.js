...
// --- Mechanics ---
  ...
  function doPoke() {
    ...
    // Only count a "poke attempt" if the fingertip actually hits the stone.
    // Poking in the air should do nothing (so you don't die just by releasing).
    // Keep the same forgiving vertical weighting for the hit-test.
    const dist = Math.hypot(dx, dy * 0.65);
    const hitStone = dist <= s.r * 1.18;
    if (!hitStone) return;

    // Determine poke "side"
    // Wider "center" zone so straight pokes are easier.
    const sideDeadzone = Math.max(12, s.r * 0.42);
    let pokeType = "S";
  ...
  function tick(now) {
    ...
    if (state.running && state.stoneObj) {
      ...
      const finger = getFingerTip();
      if (!s.touched && s.y < finger.y - s.r * 1.15) {
        gameOver();
      }
    ...
    // Background scroll: move ice DOWN to fake that we're gliding UP.
  ...
  // --- Render ---
  let iceScroll = 0;      // subtle shimmer line motion
  let iceTexScroll = 0;   // whole-ice texture scroll (downwards)
  let icePattern = null;
  ...
  function drawIce(w, h, dt) {
    ...
    // Base fill: repeating texture that scrolls DOWN
    ctx.save();
    const tile = 512;
    const offY = iceTexScroll % tile;
    ctx.translate(0, offY);
    ctx.fillStyle = icePattern || "#cfe9ff";
    // Overdraw so there are no gaps during translation
    ctx.fillRect(0, -tile, w, h + tile * 2);
    ctx.restore();
